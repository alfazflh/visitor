...
<body class="min-h-screen font-sans bg-zinc-800 dark:bg-zinc-800">
    <flux:sidebar sticky stashable class="font-sans border-e border-zinc-700 bg-zinc-900 text-white w-74 dark:border-zinc-700 dark:bg-zinc-900">
        ...
        <a href="{{ auth()->user()->role === 'admin' ? route('admin.dashboard') : route('user.registrasi') }}" class="flex items-center gap-3 px-3 py-4">
        ...
        @if (auth()->user()->role === 'admin')
            ...
            <flux:navlist.item
                ...
                :current="request()->routeIs('admin.registrasi')"
                wire:navigate
            >
                {{ __('Registrasi') }}
            </flux:navlist.item>
        ...
        @elseif (auth()->user()->role === 'user')
            <flux:navlist.item
                icon="user-plus"
                :href="route('user.registrasi')"
                :current="request()->routeIs('user.registrasi')"
                wire:navigate
            >
                {{ __('Registrasi') }}
            </flux:navlist.item>
                    ...
                    <div class="p-0 text-sm font-normal">
                        <div class="flex items-center gap-2 px-1 py-1.5 text-start text-sm">
                            <span class="relative flex h-8 w-8 shrink-0 overflow-hidden rounded-lg">
                                <span class="flex h-full w-full items-center justify-center rounded-lg bg-neutral-700 text-white dark:bg-neutral-700 dark:text-white">
                                    {{ auth()->user()->initials() }}
                                </span>
                            </span>
